...
const GameCardComponent = ({ title, genre, rating, imageUrl, className }: GameCardProps) => {
  return (
    <article 
      className={cn(
        "group relative w-full overflow-hidden bg-[#140A2E] border-4 border-[#1B123D] cursor-pointer transition-all duration-300 hover:border-neon-purple hover:-translate-y-1 shadow-[4px_4px_0_0_#000] min-h-[120px] focus-within:ring-4 focus-within:ring-neon-cyan",
        className
      )}
    >
      ...
      <div className="p-3 sm:p-4 bg-[#140A2E]">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-1 bg-[#09061B] px-2 py-1 border border-[#1B123D]">
            <Star className="w-2.5 h-2.5 text-neon-gold fill-neon-gold" aria-hidden="true" />
            <span className="text-[8px] font-pixel text-neon-gold">
              <span className="sr-only">Rating: </span>{rating.toFixed(1)}
            </span>
          </div>
        </div>
      </div>
